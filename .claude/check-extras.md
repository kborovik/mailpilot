...
In-scope cases graded deterministically: `score_replies.py` checks expected-token substring presence; false-PASS-at-worst, never false-FAIL. `expected_tokens` MUST be atomic: each token a single contiguous value the reply cannot restructure away — allowlist = {model id, bare number, number+short-unit (with optional short qualifier)}. NOT a multi-word column header, sentence fragment, or layout-dependent phrase. `select_cases.py` selection guard (>=2 tokens, len>=5) keeps real signal after brittle tokens split. `_is_brittle_inscope_token` = allowlist (not denylist). Out-scope + compare cases: `score_replies.py` emits advisory signals (token_hits, fabrication_candidates, has_table) but NOT verdicts. Sonnet judge sub-agent reads {reply body, case rubric, signals, source datasheet} → {verdict PASS|FAIL, rationale} (verdict of record for NL-shaped cases).

Trigger: `.claude/skills/mailpilot-reply-test/scripts/score_replies.py` changed.
- `rg '_is_brittle_inscope_token\|allowlist' .claude/skills/mailpilot-reply-test/scripts/score_replies.py` -> allowlist logic present (not denylist)
