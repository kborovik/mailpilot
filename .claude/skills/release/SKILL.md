...
Tag format: `v<x.y.z>` (e.g. `v0.1.0`, `v1.2.3`). SemVer only — no pre-release suffixes.

## Procedure

### §1 Parse $ARGUMENTS

- no arg -> auto bump (or baseline-mode if first release).
- `patch` or `minor` or `major` -> bump direction override.
- `x.y.z` (literal SemVer) -> pin exactly. Must match `^[0-9]+\.[0-9]+\.[0-9]+$`. Permits downgrade.
- `retag-baseline` -> recovery mode. Drops every prior tag matching §3 pattern and retags HEAD at current manifest version. Requires >= 1 prior tag. Manifest unchanged.
...
No tag -> first release (baseline mode applies in §6).
...
- **Exclude** commits whose subject matches `^chore(\([^)]+\))?: release v[0-9]+\.[0-9]+\.[0-9]+` — skill's own release commits do not count toward bump.
- no commits remaining -> exit cleanly w/ this exact shape (bypassed in retag-baseline mode):
...
- no arg and no prior tag -> **baseline mode**: target = current version. Tags the version already declared.
- no arg and prior tag exists -> auto-detect from commit subjects:
  - Any `<type>(<scope>)?!:` or body containing `BREAKING CHANGE` -> **major**
...
Preserve `#refs`, paths, identifiers, SHAs verbatim. Apply `core:steno` skill — fragments, no filler.
...
- **Bump derivation**: explicit one-liner showing rule fired — e.g. `0 breaking + 0 feat -> patch (default)`, `1 breaking -> major`, `arg "x.y.z" -> pinned`, `no prior tag & no arg -> baseline (no bump)`, `arg "retag-baseline" -> recovery (no bump, manifest unchanged)`.
- **Target tag**: `v<x.y.z>`.
- **Tags scheduled for deletion** (retag-baseline only): list every prior tag matching §3 pattern.
- **Lockfile refresh**: `uv lock` -> `uv.lock` staged with `pyproject.toml` (skipped in baseline or retag-baseline mode).
...
Wait for user confirmation. No confirm -> exit, no side effects.

### §10 Bump version in manifest

(Skip if baseline mode or retag-baseline mode — manifest already at target.)
...
(Skip if baseline mode or retag-baseline mode.)
...
Push failures on `origin main` (e.g. branch protection, remote ahead) -> bail w/ remediation note. Tag is created locally but not rolled back; user resolves remote state then re-pushes manually.
...
Assert `dist/mailpilot-<x.y.z>-py3-none-any.whl` exists. Does not exist -> bail with `ls dist/` output (build emitted unexpected name pattern -> spec drift on §V.62). Do not retry blindly.
...
Apply `core:steno` skill to release notes. Drop articles and filler, fragments and bullets, preserve identifiers / paths / `#refs` / SHAs verbatim. Tag name and version string fixed -> do not compress.
...
- `gh auth status` must report a logged-in user with `repo` scope before §15. Not logged in -> bail at §9 confirm step (caller reruns after `gh auth login`).
...
- `CHANGELOG.md` rendering — release notes live in tag annotation and GitHub release body.
