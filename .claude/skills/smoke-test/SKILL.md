...
**Derivation aids** -- Logfire scan SQL (volume / errors+warnings / agent.invoke) plus the 13-category scan checklist that feed §2 Bugs / §3 Invariants; NOT rendered to the report. Load `references/derivation-aids.md` before deriving §2/§3.

**Report format** -- the §1 Execution results matrix, the §2 Bugs and §3 Invariants block shapes, the severity + routing and auto-file matrices, and the Spec hand-off block. Load `references/report-format.md` when rendering the report.

---

## Timing

Expected total ~10-11 minutes (Phase 0 once, run loop once, no reset between scenarios). Per-phase duration table and compare/burst SLA notes: load `references/timing.md`.
