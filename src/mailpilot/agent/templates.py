...
_DEFERRED_TASK_TASK = (
    ...
    "when a human says wrong person, when an automated reply says the person "
    "retired or left the company, that their last day was (already past), or "
    "when an automated reply says the email address has changed, to update "
    "your records, or hard-redirects to a new address -- stop further touches "
    "to the enrolled address even if From uses a different local-part; put the "
    "redirect, referral addresses, named successors without emails, and the "
    "new email (when present) in the note so campaign review can re-target; "
    "never enroll the From alias or any new address yourself. Out-of-office "
    "and temporary absence auto-replies are different: call noop (the system "
    "pauses and resumes the sequence; leave enrollment open) "
    "-- do not conclude and do not reply. A past last-day auto-reply is "
    "left-company, not out-of-office -- do not pause on that date. "
...
# (§V.131, closes §B.116). It is first-person singular (`I`), not we/our team
# (§B.139) -- the live sender is one person. It is code-defined -- never
# model-generated and never partial -- so the grounding risk the agent failed
# on (a fabricated or truncated answer, §V.45 / §V.130) cannot reach the
# sender; the inbound sender gets a brief receipt instead of silent NO_REPLY.
# ``run._handle_agent_failure`` sends this string verbatim via
# ``email_ops.reply_email``; an outbound first reach-out failure (no inbound
# email_id) stays silent, and an OOO / auto-reply inbound is also silent. The
# model never sees this string, yet it stays ASCII-only per §C (code-defined
# content, not agent-generated).
