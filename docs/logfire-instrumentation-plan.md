...
| [agent/tools.py](../src/mailpilot/agent/tools.py)       | No custom spans -- `instrument_pydantic_ai()` auto-creates tool spans with arguments                             | None                                                                                      |
...
| `agent.*`    | 1          | `agent.invoke` (tool spans come from `instrument_pydantic_ai()` as `running tool`) |
...
### `agent/invoke.py` + `agent/tools.py` -- DONE (landed in PRs #26, #56, #74)

`agent.invoke` span carries `result`, `agent_reasoning`, `tool_calls` attributes. Agent reasoning is also persisted to `task.result` JSONB column. Tool-level spans are auto-generated by `logfire.instrument_pydantic_ai()` (as `running tool: <name>`) with full tool arguments -- custom `agent.tool.*` spans were removed in #74 to eliminate duplication.
...
| ~~P2~~   | ~~#56, #72~~ | ~~`agent/tools.py` -- tool call spans (custom spans removed; auto-instrumented by Pydantic AI)~~ | **DONE** |
