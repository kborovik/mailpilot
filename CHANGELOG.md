...
- `mailpilot tui` shows one table per tab. `/` opens a compact
  ...
  Customers, H2 Contacts as a DataTable of `company view --full`
  extras (email, name, title), H2 Notes, H2 Sources last
  (clickable URLs). Enter on a Contacts row opens that contact.
  Escape from that contact returns to the start page. No H1
  Profile. Timezone and the identity dump (id, timestamps,
...
- Outbound cadence can ship per-touch copy in `[[touch_copy]]` on the
  ...
  CompanyView with no LLM call; missing N stays compose-only. Closed
  placeholders: `first_name`, `last_name`, `full_name`, `title`,
  `email`, `company_name`, `company_domain`. Import hashes
...
- Drop the leftover `httpx` runtime dependency after the Anthropic
  transport switch to `httpx2`. Anthropic LLM read-timeouts now
  raise `httpx2.ReadTimeout` (still terminal, not retried).
