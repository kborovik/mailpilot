...
T148|.|impl §V.107(+) — shared `_resolve_account_id(connection, account_id, account_email)` resolver (exactly-one guard -> validation_error; email -> get_account_by_email case-insensitive; unknown -> not_found §V.94); make `--account-id` optional + add `--account-email` on `email send|reply` + `workflow create|export|import`; TDD|V107
