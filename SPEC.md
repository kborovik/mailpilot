...
- cli: `mailpilot <noun> <verb> [args]` -> JSON on stdout, exit code 0 or 1, errors to stderr.
  ...
  - CLI API standard (§V.107, §V.115, §V.116) — verb roles: Read {list, search, view, stats (read-only per-campaign aggregate §V.132)}; Mutate {create, add, remove, update, disable, enable} (`create` = new entity from own fields; `add`/`remove` = link/unlink to an owner (`note remove` exception = clear an owner's notes, the sole hard-delete §V.14); `disable` = soft-retire, `enable` = clear the soft-disable; no `delete`, no hard-delete §V.10/§V.114 except `note remove` clears notes §V.14); Lifecycle {start, stop, cancel, retry, enrollment `run`} carry per-entity state guards (`invalid_state`, §V.109); Action {send, reply, sync} = external Gmail effect; Schema/catalog {init, migrate, check, export, import}. `activity create` -> `activity add` (attaches event to a required owner). `list` filter flags = six-family taxonomy (§V.115); error envelope `error` code = closed vocabulary — domain codes {`not_found`, `invalid_state`, `missing_filter`, `already_exists`, `schema_migration_pending`, `schema_drift`, ...} around the §V.54 constraint/validation half, new code = spec change.
...
- config (`src/mailpilot/settings.py`): `database_url`, `anthropic_api_key`, `anthropic_model` (default `claude-sonnet-4-6`), `anthropic_base_url` (default `https://api.anthropic.com`; override -> Anthropic-compatible endpoint e.g. `https://api.novita.ai/anthropic`; not secret per §V.86), `anthropic_thinking` (default `adaptive`; in {unset, `adaptive`}; workflow agent only, enables extended thinking per §V.130), `anthropic_effort` (default `high`; in {unset, `low`, `medium`, `high`, `xhigh`, `max`}; workflow agent only, reasoning effort per §V.130; `xhigh` Opus 4.7+ only, errors on Sonnet 4.6), `anthropic_max_tokens` (default `32768`; int; workflow agent only, output-token budget always passed per §V.130), `google_application_credentials`, `google_pubsub_topic`, `google_pubsub_subscription`, `logfire_token`, `logfire_environment` in {`development`, `production`}, `run_interval` (default 60s), `max_concurrent_tasks` (default 10).
...
V14: activity append-only — INSERT only, no update/delete fns; note INSERT-only EXCEPT operator bulk-clear `note remove --contact-email|--company-domain` (§I) hard-deletes all of one owner's note rows (no single-note delete, no note update), operator-only NOT an agent tool, activity trail untouched
...
V91: tag/note mutation + its activity row commit in one transaction — both or neither; `note remove` (§V.14) deletes note rows only + writes no activity — prior note_added rows survive as the append-only trail
...
V122: campaign-test delivery keyed on recipient alias (inbound{N}@lab5.ca), not agent-written subject — → .claude/check-extras.md §V.122
