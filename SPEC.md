...
T113|x|impl §V.3(+) per §B.73 — Logfire console exporter ! write stdout: configure_logging (cli.py:66) set ConsoleOptions(output=sys.stderr) so warn/debug console lines land stderr (all flags incl --debug); stdout stays JSON-only. Recurrence-guard test: warn-emitting CLI cmd (schema-drift path) -> assert stdout parses JSON and warn text in stderr not in stdout (capsys). Scope = grep `rg -n 'ConsoleOptions' src/mailpilot/` -> all hit carries output=sys.stderr. Failing test first per TDD.|V3,B73
T114|x|impl §V.97(+) per §B.74 — add deferred = stale-count - processed to the agent-emitted run summary in both lead-companies + lead-contacts "Run summary" sections when the batch gate caps below stale-count; deferred 0 or omitted when all processed; skill-prose only, no src/ change. Scope = grep `rg -n 'Run summary' .claude/skills/lead-companies/SKILL.md .claude/skills/lead-contacts/SKILL.md` -> each Run-summary section mandates the deferred field. Verify: a capped sample run emits deferred.|V97,B74
T115|x|impl §V.98(+) per §B.75 — seed_companies.py collision branch: on company-create duplicate_key, fetch the owning row's name; name diverges from incoming CSV display name -> append to collapsed (not bare existing); mirror in §Stage: seed + Run summary prose. Scope = grep `rg -n 'duplicate_key|collapsed' .claude/skills/lead-companies/scripts/seed_companies.py .claude/skills/lead-companies/SKILL.md` -> collision branch + prose record name-divergent merges. Verify: re-seed a divergent-name redirect -> collapsed entry, not bare existing.|V98,B75
T116|x|impl §V.5(+): contact list envelope + filters — ContactSummary (+ search_contacts mirror) gains title + company_domain via single LEFT JOIN company ON company_id (NULL when company_id NULL; no N+1 per §V.5); new filters --min-email-confidence (composes w/ --max-email-confidence -> email_confidence >= MIN AND <= MAX, NULL-score rows excluded by both bounds per §V.95) + --company-domain (matches joined company.domain) + --title (substring ILIKE); failing test first per TDD|V5,V95
T117|x|impl §V.95(+) per §B.76 — contact review filter surfaces NULL email_confidence: database.py:1010 `(email_confidence <= %(max_email_confidence)s OR email_confidence IS NULL)`; fix list_contacts docstring database.py:991-994 (NULL = unknown = high-risk, surfaced not skipped). Recurrence-guard test: seed a NULL-confidence contact -> assert `contact list --max-email-confidence 70` returns it. Scope = grep `rg -n 'email_confidence <=' src/mailpilot/database.py` -> filter carries the OR IS NULL disjunct. Failing test first per TDD.|V95,B76
...
B74|2026-06-13|lead-companies on theirstack.csv (stale=23): operator chose First-10 at the batch gate -> 13 rows left profile IS NULL, but run-summary envelope {created,existing,enriched,skipped,failed} carries no deferred count -> operator reading only JSON cannot see 13 rows need a follow-up run; symmetric in lead-contacts. Recurrence-class: any batch-gate cap (First-N or --limit < stale) silently drops the remainder. Fix: §V.97(+) + §T.114.|V97
B75|2026-06-13|lead-companies seed collapsed accumulator catches only intra-batch apex merges; a CSV row redirecting onto a previously-seeded apex w/ a divergent name lands silent in existing — confirmed: sole whitecapsupply.com row named "National Concrete Accessories" (acquired by White Cap), surfaced only as existing:2, hiding the entity merge. Recurrence-class: any re-run / new dump redirecting onto a prior apex w/ divergent name. Fix: §V.98(+) + §T.115.|V98
B76|2026-06-13|lead-contacts seeded 35 contacts all email_confidence NULL (Bouncer unknown — catch-all + SMTP-protected domains); §V.96 review path `contact list --max-email-confidence 70` returned 0/35 — database.py:1010 `email_confidence <= N` excludes NULL (SQL three-valued logic) + docstring database.py:991-994 codified the exclusion ("no signal to review") contradicting §V.96 NULL-flags-risk; operator sees empty review list, ships unverified addresses. Recurrence-class: any admit-all NULL-sentinel risk policy w/ inequality review filter -> flagged-NULL rows unreviewable. Fix: §V.95(+) + §T.116.|V95
