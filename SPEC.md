...
T123|.|impl §V.100(+) sibling-DRY — extract near-verbatim lead-companies/lead-contacts shared prose (Conventions: envelope/stdout-parsing/ASCII; batch-gate logic; Next-block format) -> `references/lead-pipeline-conventions.md` both skills cite, not per-skill copies. Scope = grep `rg -n 'Conventions|batch gate|Next block' .claude/skills/lead-companies/SKILL.md .claude/skills/lead-contacts/SKILL.md` -> shared block replaced w/ references pointer. Skill-prose only, no src/ change.|V100
