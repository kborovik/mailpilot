...
V138: company cohort pipeline status — `company list --status` Enum ∈ {ready, needs_contacts, needs_profile, disabled}; rules: ready = has_profile + contact_count ≥ 1 + not disabled; needs_contacts = has_profile + contact_count = 0 + not disabled; needs_profile = !has_profile + not disabled; disabled = disabled_reason set; Enum family §V.115; `--status disabled` overrides default hide; AND-composes w/ `--tag`/`--no-tag`/`--min/max-contacts`/`--has-profile`/`--include-disabled`; rules in --skill/help (zero SPEC cites §V.111); no `company audit` verb
