...
V59: /demo-test = non-destructive liveness probe of public demo — outbound@lab5.ca -> hello@lab5.ca, warm state, subject [DEMO-<HHMMSS>] fresh-randomized; PASS = gates G1/G2/G3 + zero error/warn — gates → .claude/check-extras.md §V.59
V61: reply-latency verdict from agent.invoke span; CLI poll = round-trip check; two-budget split sla_agent_seconds + sla_delivery_seconds — thresholds → .claude/check-extras.md §V.61
...
V70: burst retry-rate contract — tool_errors/agent.invoke ratio <= 5% in N-burst window; breach -> §V.41/§V.57/§V.42 investigation — measurement detail → .claude/check-extras.md §V.70
...
T128|.|impl §V.42(+) _BASE pipe-table mandate — replace _BASE "Email bodies may use Markdown formatting (headers, bold, tables)" in src/mailpilot/agent/templates.py w/ prescriptive: when reply body carries product specs (model numbers, flow rates, dimensions, capacities), use GFM pipe table w/ header row + |---| separator; space-aligned spec blocks rejected by §V.42 lint. Add test asserting _BASE carries the mandate. Scope = grep `rg -n 'pipe.table\|may use Markdown' src/mailpilot/agent/templates.py` -> mandate present|V42
T129|.|reduce smoke-test Scenario C burst N=8/P=8 -> N=4/P=4, mix 4-in/2-out/2-cmp -> 2-in/1-out/1-cmp, across .claude/skills/smoke-test/ (SKILL.md + references/report-format.md + references/timing.md): scenario table (:24), C1 payload-gen + distinct-count check (8->4) + mix gate (2 qa-in/1 qa-out/1 qa-cmp), C2 "Fire 4 sends at P=4" (seq 0 3, jobs>=4) + 4-row gate, C3 4-reply poll, C4.a n_invokes/n_distinct 8->4 + n_compare/n_noncompare re-derived from 2/1/1 mix, C4.b overlap_pairs floor re-derived for N=4 (C(4,2)=6 max -> ">=10" impossible, set >=3), retry-rate N-note, report-format C-matrix + timing C-rows. Scope = grep `rg -n 'P=8|N=8| x8|8 emails|8 sends|8 replies|overlap_pairs >= 10|4 in-scope|4 qa-in' .claude/skills/smoke-test/` -> zero stale N=8 refs. Skill-prose only, no src/.|-
