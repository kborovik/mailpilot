...
T125|.|impl §V.102(+) frontmatter — add scoped `allowed-tools` + `argument-hint` to demo-test + smoke-test SKILL.md (both set neither; smoke-test runs `make clean` + sends real email so a scoped allowed-tools is a cheap safety rail). Scope = grep `rg -n 'allowed-tools|argument-hint' .claude/skills/demo-test/SKILL.md .claude/skills/smoke-test/SKILL.md` -> both keys present. Skill-prose only.|V102
