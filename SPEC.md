...
- cli: `mailpilot <noun> <verb> [args]` -> JSON on stdout, exit code 0 or 1, errors to stderr.
  ...
  - `workflow import` accepts `--file <path>`: `.toml` single-workflow catalog entry | directory (glob `*.toml`); TOML-only, no JSON, no stdin (§V.103); import enforces `name` = kebab file-stem, globally unique; def fields `{name, template, theme, goal, instructions}` import-only — `workflow update` mutates non-def fields only (status, account binding), rename = rename file + re-import (§V.103). `workflow export --account-email <addr> --out-dir <dir>` writes one `*.toml`/workflow (def fields, name-sorted) + JSON status envelope on stdout (§V.103, §V.3).
  - `workflow check` (§V.134) = read-only wording-integrity check, mirrors `db check`: live 2-way SHA-256 over wording fields `{template, theme, goal, instructions}` per row vs each `workflows/*.toml` by name; states {`in_sync`, `file_ahead`, `file_missing`, `row_orphan`}; report-only `ok:true`, not a deploy gate; envelope `{"workflow_check": {...}, "ok": true}` (aggregate not a workflow row).
...
V134: `workflow check` = read-only wording-integrity check, mirrors `db check` structure (live 2-way SHA-256, no stored column) over canonical serialization of wording fields {template, theme, goal, instructions} per workflow row vs each live `workflows/*.toml`, compared by `name`; states in {`in_sync`, `file_ahead` (file edited, re-import due), `file_missing` (file gone), `row_orphan` (row, no file)}; report-only envelope {"workflow_check": {...}, "ok": true} (aggregate not a workflow row, cf §V.132), not a deploy gate; hash excludes `name` (rename ! drift); def fields import-only (§V.103) so mismatch = file ahead, no `row_ahead` state; modeled on §V.108 schema-hash pattern
...
T202|.|impl §V.90(∆) + §V.107(∆) — workflow joins keyed entities, natural key `name`; migration 009 + schema.sql: `workflow.name` global UNIQUE (drop `(account_id, name)` composite) + kebab CHECK, one-time normalize existing names to kebab; polymorphic resolver resolves workflow by name case-insensitively; workflow leaves keyless list|V90,V107,V108,V103
T203|.|impl §V.103(∆) — `workflow import` enforces `name` = kebab file-stem + global unique; def fields {name,template,theme,goal,instructions} import-only; `workflow update` restricted to non-def fields (status, account binding)|V103,V107,V90
T204|.|impl §V.134(+) + §I — `workflow check` verb: 2-way live SHA-256 over wording fields {template,theme,goal,instructions} per row vs `workflows/*.toml` by name; states {in_sync,file_ahead,file_missing,row_orphan}; `{"workflow_check"}` envelope|V134,V103,V107,V4,V54
