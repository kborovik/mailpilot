...
`llm_provider` in {`anthropic`, `xai`} (default `xai`) selects factory branch for **both** classifier + workflow agent via public `build_model(settings, *, role)`. `require_active_provider_key` runs once at `build_model`; inner `_build_anthropic_model` / `_build_xai_model` do not re-call. Active-provider API key required @ each `mailpilot run` tick before drain (and still @ model build). Host-config key error (closes §B.141, §B.152): missing/empty (preflight `require_active_provider_key`) or present-but-wrong (xAI `ModelAPIError` "Incorrect API key" / Anthropic 401 at first model call) → skip remaining drain that tick; process stays up; zero due tasks claimed or marked `failed`; already-claimed in-flight stays `pending` (no attempt_count bump; no §V.131 fallback ACK). Operator stderr: one `error` line naming `mailpilot config set xai_api_key` (provider=xai) or `mailpilot config set anthropic_api_key` (provider=anthropic); `logfire.error` not `logfire.exception` (zero console Traceback). Model-build remains fail-closed (no provider fallthrough) if preflight skipped. Inactive-provider keys may be empty. Keys live on `app_config`; `MAILPILOT_*` API-key env vars are not sources. Dep: `pydantic-ai-slim[anthropic,xai]`.
...
- xAI path: `rg 'api_host=' src/mailpilot/agent/model.py` -> zero hits (never pass `api_host`)
...
- `rg 'mailpilot config set xai_api_key|mailpilot config set anthropic_api_key' src/mailpilot/` -> missing/invalid-key error names config set
...
- fixture: due T1 batch + injected invalid-key error → zero tasks written `failed`; in-flight stays pending
...
- `rg 'touch_copy' src/mailpilot/cli/workflow.py src/mailpilot/database/workflow.py` -> import-only hashed def field
...
- `rg 'touch_copy' src/mailpilot/database/workflow.py` -> hashed with def fields
...
Workflow def fields `touches` + `touch_interval_days` (nullable pair, §V.103; NULL = single-touch, no auto follow-up). Cadence engine (`cadence.py`) owns schedule math (weekend -> Monday roll) + touch scheduled_at — system-computed only, §V.129 exempt path. Successful touch-N send -> harness creates touch-N+1 task w/ context {touch: N+1, prior_email_id}. Final touch -> §V.186 helper `contact_later` "sequence exhausted" (no re-enrollment task, no agent turn). Touch runs (context.touch present | trigger enrollment_schedule | enrollment_run) = compose-only agent when no `touch_copy` row for N: output_type TouchMessage {subject: str|None, body: str}, zero tools; output validator (bounded ModelRetry, same agent retry budget): first-touch subject require — new-thread touch (touch 1 / no prior outbound to reply on) → after strip `subject` ! non-empty; None/"" /whitespace-only → ModelRetry (message: subject required for new thread); follow-up that continues existing thread may leave subject empty (harness reply_email keeps thread subject) (closes §B.127); body format lint retired (§V.42 / §B.128); harness sends via `_deliver_touch` (email_ops + cadence). Copy-row N = `copy_for_touch` then `render_touch_copy` then `_deliver_touch`, never `build_model`/prompt, span llm_requests=0 (§V.194). Dispatch above `build_model`. 1 LLM call per no-row touch, send structural (§V.120). Outbound task|email triggers keep the tool loop; inbound unchanged and never reads `touch_copy` (§V.44 registry owns both shapes). prior_email_id from task context; absent -> enrollment's latest outbound email. NULL-cadence belt: touch >= 2 vs NULL cadence -> reschedule +1h + operator warn (§V.25 shape); touch 1 vs NULL -> send + schedule nothing. create_task stays bound for reply-branch soft follow-ups. Cadence + after-touch prose live in def fields; wording for a copy-row lives in `touch_copy`, never TOML instructions.
...
- `rg 'copy_for_touch' src/mailpilot/agent/invoke.py src/mailpilot/touch_copy.py` -> copy-row dispatch before `build_model`
...
Task grain: default union pending + failed-unsent + stuck (alongside, not pending-only); sort pending scheduled_at ASC then failed then stuck; table+JSON cols {workflow_name, company_domain, contact, email, touch, attempts, next_at, kind, reason} (drop when, trigger, state); kind ∈ {pending, failed, stuck}; reason = result.reason string or empty (pending empty; failed stored; stuck from latest failed task when present else empty); hide UUIDs on table; JSON ? include task_id + enrollment_id; `--limit` default 100 per kind (pending/failed/stuck each capped; failed+stuck not starved when pending fills a shared cap); `--overdue` = pending + scheduled_at < now (pending-kind only); `--failed` = failed-unsent only; `--stuck` = stuck-enrollment only; `--overdue`/`--failed`/`--stuck` XOR on `--detail` (2+ → validation_error); without `--detail` ignored. Distinct from §V.155 enrollment/report `--stuck` (no 24h SLA; includes attempt_count=0 failed-unsent). --skill one-call `show queue --detail --format json` (or `--detail --failed`) replaces `show queue` then `task list --status failed` for send-window outage classify; keep `show queue --format json` replacing workflow review for empty-queue. Help zero SPEC cites §V.111.
...
- `rg 'kind|reason' src/mailpilot/models.py` -> QueueTaskRow kind+reason
- `rg 'show queue --detail' src/mailpilot/SKILL.md` -> one-call drops task list --status failed
...
enrollment add scheduled-batch apply — source XOR `--file` | `--tag` (no `--dry-run`) | `--contact-email` (single unchanged §V.32 unless `--exclude-peer`); `--file` exclusive w/ `--tag` + `--contact-email`; apply source (`--file` or `--tag` w/o dry-run) ! `--scheduled-at` ISO future (else `validation_error`); inbound workflow → `invalid_state`; `--file` = JSON array of email strings or `{email, scheduled_at?}` objects; missing path → `not_found`; bad JSON / missing email → `validation_error`; unknown email → `not_found` zero writes; `--tag` expand = §V.150 candidate set (drop already-enrolled this workflow + self-loop §V.33 + disabled company/contact §V.114); `--min-contacts` same pre-expand filter; `--exclude-peer` drops contacts w/ other-workflow active enrollment; `--exclude-peer` also valid on `--contact-email` (same drop); `--limit` / `--company-atomic` still `--file|--tag` (one-off `--exclude-peer` does not lift those); one-off+`--exclude-peer` → `enrollment_batch` `source=contact` (`scheduled_at` = flag ISO or null); peer-active → skip no write count=0 excluded.peer=1; enroll → count=1 enrolled[0] action as §V.32; `--contact-email` without `--exclude-peer` stays entity envelope unchanged; `--limit N` (N≥1) caps included seats — w/o `--company-atomic` hard cap first N in company_domain then email order; w/ `--company-atomic` soft cap: take whole company atoms in that order, last atom may exceed N, never split a domain; `--company-atomic` ! same calendar day (offset of `--scheduled-at` or per-company file override) for every included seat on a domain; file row `scheduled_at` overrides flag per contact; `--company-atomic` + conflicting per-row instants on one domain → `validation_error`; one txn; per-row first-touch writer = §V.32 (insert-once; last-write-wins on `--file` existing emails_sent=0 first-reach; never move T2+); tag apply never restamps already-enrolled; envelope `{"enrollment_batch":{workflow, scheduled_at, source, tag?, limit?, company_atomic, count, enrolled:[{email, company_domain, enrollment_id, scheduled_at, action}], excluded:{disabled_companies, already_enrolled, self_loop, disabled_contacts, peer, over_limit, not_found}},"ok":true,"record_count":count}` action ∈ {created, scheduled_first_send, unchanged} source ∈ {file, tag, contact}; `--file|--tag --dry-run` + packing flags = preview packed set (no writes; `enrollment_preview` §V.150 + excluded.peer/over_limit); `--tag` w/o `--dry-run` and w/o `--scheduled-at` → `validation_error`; `--limit` <1 → `validation_error`; --skill one-call recipe replaces N-call `--contact-email` loop; `--contact-email --exclude-peer` one-call (no prior `enrollment list`); help documents `--exclude-peer` w/ `--contact-email`; help zero SPEC cites §V.111
...
- `rg 'source=.contact|"contact"' src/mailpilot/models.py src/mailpilot/cli` -> one-off exclude-peer source present
...
## §V194 — touch-copy-dispatch

Outbound touch N is template iff `copy_for_touch(workflow, n)` returns a row. Row → `render_touch_copy` (ContactView+CompanyView §V.135) → TouchMessage → `_deliver_touch`. Never `build_model`/prompt. Span `llm_requests=0` token attrs 0. No row → compose-only LLM then `_deliver_touch` §V.136. Inbound classify+invoke never read `touch_copy`. `[[touch_copy]]` JSONB import-only hashed w/ §V.103/§V.134. Dispatch above `build_model` in `invoke_workflow_agent`. §V.47 gate unchanged (copy-row not a key bypass). `template` registry + CLI unchanged.

Trigger: `src/mailpilot/touch_copy.py` or `src/mailpilot/agent/invoke.py` changed.
- `rg 'def copy_for_touch' src/mailpilot/touch_copy.py` -> lookup helper
- `rg 'def render_touch_copy' src/mailpilot/touch_copy.py` -> render helper
