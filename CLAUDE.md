...
`search_*` has the same contract as `list_*` -- use `<Entity>Summary` return types and SELECT only summary columns. Search matches against all indexed text fields (e.g. `body_text`, `objective`, `recipients`) but those fields are not returned in the projection. Callers who need full records on a search hit use `get_<entity>(id)` to hydrate.
