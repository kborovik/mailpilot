...
Per §B.71: the ``database.py`` connect-fail path shipped a ``logfire.exception``
...
``cli.py`` is intentionally excluded: its ``logfire.exception`` sites live in
...
def test_logfire_exception_paired_with_operator_error(module_name: str) -> None:
    ...
    tree = ast.parse((_SRC_ROOT / module_name).read_text())
    ...
    for node in ast.walk(tree):
        ...
        if scope is None or not any(
            ...
            unpaired.append(getattr(node, "lineno", -1))

    assert not unpaired, (
        f"{module_name}: logfire.exception at line(s) {unpaired} lack a paired "
...
def test_sweep_actually_inspects_exception_sites() -> None:
    ...
    total = sum(
        _count_logfire_exceptions(ast.parse((_SRC_ROOT / name).read_text()))
